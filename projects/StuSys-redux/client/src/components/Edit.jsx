 ...
 * @LastEditTime: 2024-12-02 17:58:54
...
import { addStudentAsync,updateStudentAsync } from '../redux/slices/student'
const studentTemp = {
    name: '',
    age: '',
    phone: '',
...
function Edit(props) {
    ...
    useEffect(() => {
        if (id) {
            const item = studentList.find(item => item.id === id) || studentTemp
            setStudent(item)
        } else {
    ...
    const handleSubmitInfo = async (e) => {
        ...
        if (id) {
            await dispatch(updateStudentAsync({id, student}))
        } else {
